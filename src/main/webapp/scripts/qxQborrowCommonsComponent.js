if(typeof qborrowPresent == 'undefined'){
	// Variabile globale dove registrare la lista dei viewer presenti nella pagina.
	qborrowPresent = new Array();
    // Creazione del modulo e delle relative dipendenze
    var dependencyArray = ['ui.bootstrap','ngAnimate', 'ngMessages','oitozero.ngSweetAlert','ui.bootstrap.datetimepicker'];
    
    var qborrowApp = angular.module('qborrow', dependencyArray);
    ...
    function qxValidationError(data, form, $timeout, scopeController) {
    	$timeout( function() {
        	...
        	for (i = 0; i < data.errors.length; i++) { 
        		...
        		if(form[err.property] != undefined) {
        			form[err.property].$setValidity(err.message, false); 
        		} else {
    ...
    qborrowApp.run(["$templateCache", function($templateCache) {
